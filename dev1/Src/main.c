...
STATIC_ASSERT(sizeof(FIRMWARE_NAME) <= 13, firmware_name_too_long);
...
if(zero_crosses < 5){
	...
	min_bemf_counts_down = TARGET_MIN_BEMF_COUNTS;
        ...
        if (tenkhzcounter > LOOP_FREQUENCY_HZ) { // 1s sample interval 10000
            ...
            case 1:
                send_extended_dshot = 0x02 << 8 | degrees_celsius;
                dshot_extended_telemetry = 2;
                break;
            case 2:
                send_extended_dshot = 0x06 << 8 | (uint8_t)actual_current / 50 ;
                dshot_extended_telemetry = 3;
                break;
            case 3:
                send_extended_dshot = 0x04 << 8 | (uint8_t)(battery_voltage / 25);
